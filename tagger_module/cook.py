...
from chess import (
    ...
    Piece,
    PieceType,
    square_distance, Move,
)
from chess import KING, QUEEN, ROOK, BISHOP, KNIGHT, PAWN
from chess import WHITE, BLACK
from chess.pgn import ChildNode

from tagger_module import util
from tagger_module.model import Puzzle, TagKind
import tagger_module.util
from tagger_module.util import material_diff
...
def advanced_pawn(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
...
def double_check(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
...
def sacrifice(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    ...
    initial = diffs[0]
    for node, d in zip(puzzle.mainline[1::2][1:], diffs[1::2][1:]):
        if d - initial <= -2:
            return node.move, not any(n.move.promotion for n in puzzle.mainline[::2][1:])
    return None, False


def x_ray(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
...
def fork(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2][:-1]:
...
def hanging_piece(puzzle: Puzzle) -> tuple[None, bool] | tuple[Move, bool]:
...
def trapped_piece(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2][1:]:
...
def overloading(puzzle: Puzzle) -> tuple[None, bool]:
    return None, False


def discovered_attack(puzzle: Puzzle) -> tuple[Any, bool] | tuple[None, bool] | tuple[Move, bool]:
    discovered_check_move, discovered_check_bool = discovered_check(puzzle)
    if discovered_check_bool:
        return discovered_check_move, True
    for node in puzzle.mainline[1::2][1:]:
...
def discovered_check(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
        board = node.board()
...
def quiet_move(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
...
def defensive_move(puzzle: Puzzle) -> tuple[None, bool] | tuple[Move, bool]:
    ...
    if util.attacked_opponent_pieces(node.board(), node.move.to_square, puzzle.pov):
        return None, False
    # no advanced pawn push
    return node.move, not util.is_advanced_pawn_move(node)


def check_escape(puzzle: Puzzle) -> tuple[None, bool] | tuple[Move, bool]:
    for node in puzzle.mainline[1::2]:
        if node.board().is_check() or util.is_capture(node):
            return None, False
...
def attraction(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
...
def deflection(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2][1:]:
...
def exposed_king(puzzle: Puzzle) -> tuple[None, bool] | tuple[Move, bool]:
    if puzzle.pov:
        pov = puzzle.pov
...
def skewer(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2][1:]:
        prev = node.parent
        assert isinstance(prev, ChildNode)
...
def self_interference(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
...
def interference(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
...
def intermezzo(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2][1:]:
        if util.is_capture(node):
...
def pin_prevents_attack(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
        board = node.board()
...
def pin_prevents_escape(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
        board = node.board()
...
def attacking_f2_f7(puzzle: Puzzle) -> tuple[Move, bool | Any] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
        square = node.move.to_square
...
def kingside_attack(puzzle: Puzzle) -> bool:
    return "move", side_attack(puzzle, 7, [6, 7], 20)


def queenside_attack(puzzle: Puzzle) -> bool:
    return "move", side_attack(puzzle, 0, [0, 1, 2], 18)
...
def clearance(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2][1:]:
        board = node.board()
...
def en_passant(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
        if (
...
def castling(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
...
def promotion(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
...
def under_promotion(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2]:
        if node.board().is_checkmate():
            return (node.move, True) if node.move.promotion == KNIGHT else (None, False)
        elif node.move.promotion and node.move.promotion != QUEEN:
            return node.move, True
    return None, False


def capturing_defender(puzzle: Puzzle) -> tuple[Move, bool] | tuple[None, bool]:
    for node in puzzle.mainline[1::2][1:]:
        board = node.board()
