...
from Evaluation.global_functions import all_squares, colorflip, board
...
def candidate_passed(pos, square=None):
    if square is None:
        return sum(candidate_passed(pos, sq) for sq in all_squares(pos))
    ...
    lever = (board(pos, square.x - 1, square.y - 1) == "p") + (board(pos, square.x + 1, square.y - 1) == "p")
    leverpush = (board(pos, square.x - 1, square.y - 2) == "p") + (board(pos, square.x + 1, square.y - 2) == "p")
...
def king_proximity(pos, square=None):
    if square is None:
        return sum(king_proximity(pos, sq) for sq in all_squares(pos))
...
def passed_block(pos, square=None):
    if square is None:
        return sum(passed_block(pos, sq) for sq in all_squares(pos))
...
def passed_file(pos, square=None):
    if square is None:
        return sum(passed_file(pos, sq) for sq in all_squares(pos))
...
def passed_rank(pos, square=None):
    if square is None:
        return sum(passed_rank(pos, sq) for sq in all_squares(pos))
...
def passed_leverable(pos, square=None):
    if square is None:
        return sum(passed_leverable(pos, sq) for sq in all_squares(pos))
    ...
    for i in [-1, 1]:
        ...
        if (board(pos, square.x + i, square.y + 1) == "P" and "pnbrqk".find(board(pos, square.x + i, square.y)) < 0
...
def passed_mg(pos, square=None):
    if square is None:
        return sum(passed_mg(pos, sq) for sq in all_squares(pos))
...
def passed_eg(pos, square=None):
    if square is None:
        return sum(passed_eg(pos, sq) for sq in all_squares(pos))
