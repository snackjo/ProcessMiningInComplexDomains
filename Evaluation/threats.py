...
def safe_pawn(pos, square=None):
    if square is None:
        return sum(safe_pawn(pos, sq) for sq in all_squares(pos))
...
def threat_safe_pawn(pos, square=None):
    if square is None:
        return sum(threat_safe_pawn(pos, sq) for sq in all_squares(pos))
...
def weak_enemies(pos, square=None):
    if square is None:
        return sum(weak_enemies(pos, sq) for sq in all_squares(pos))
...
def minor_threat(pos, square=None):
    if square is None:
        return sum(minor_threat(pos, sq) for sq in all_squares(pos))
...
def rook_threat(pos, square=None):
    if square is None:
        return sum(rook_threat(pos, sq) for sq in all_squares(pos))
...
def hanging(pos, square=None):
    if square is None:
        return sum(hanging(pos, sq) for sq in all_squares(pos))
...
def king_threat(pos, square=None):
    if square is None:
        return sum(king_threat(pos, sq) for sq in all_squares(pos))
...
def pawn_push_threat(pos, square=None):
    if square is None:
        return sum(pawn_push_threat(pos, sq) for sq in all_squares(pos))

    if "pnbrqk".find(board(pos, square.x, square.y)) < 0:
        return 0

    for ix in [-1, 1]:
        if (board(pos, square.x + ix, square.y + 2) == "P" and board(pos, square.x + ix, square.y + 1) == "-" and
                board(pos, square.x + ix - 1, square.y) != "p" and board(pos, square.x + ix + 1, square.y) != "p" and
        ...
        if (square.y == 3 and board(pos, square.x + ix, square.y + 3) == "P" and
                board(pos, square.x + ix, square.y + 2) == "-" and board(pos, square.x + ix, square.y + 1) == "-" and
                board(pos, square.x + ix - 1, square.y) != "p" and board(pos, square.x + ix + 1, square.y) != "p" and
                (attack(pos, {'x': square.x + ix, 'y': square.y + 1}) or
                 not attack(colorflip(pos), {'x': square.x + ix, 'y': 6 - square.y}))):
            return 1
...
def slider_on_queen(pos, square=None):
    if square is None:
        return sum(slider_on_queen(pos, sq) for sq in all_squares(pos))
...
def knight_on_queen(pos, square=None):
    if square is None:
        return sum(knight_on_queen(pos, sq) for sq in all_squares(pos))
    ...
    if queen_count(pos2) != 1 or board(pos, square.x, square.y) == "P" or \
            board(pos, square.x - 1, square.y - 1) == "p" or board(pos, square.x + 1, square.y - 1) == "p" or \
...
def restricted(pos, square=None):
    if square is None:
        return sum(restricted(pos, sq) for sq in all_squares(pos))
...
def weak_queen_protection(pos, square=None):
    if square is None:
        return sum(weak_queen_protection(pos, sq) for sq in all_squares(pos))
